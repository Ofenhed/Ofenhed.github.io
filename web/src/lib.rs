    ...
    #[cfg(any(feature = "debug-reload-panics", not(debug_assertions)))]
    {
        ...
        SET_HOOK.call_once(|| {
            ...
            std::panic::set_hook(Box::new(move |info| {
                static RECOVER_BY_RELOAD: Once = Once::new();
                RECOVER_BY_RELOAD.call_once(|| {
                    ...
                    'reload_page: {
                        if let Some(location) = document().location() {
                                ...
                                if let Ok(hash) = location.hash() {
                                    if hash.strip_prefix('#').unwrap_or(&hash) == reload_keyword {
                                        leptos::logging::error!(
                                            "Reloading did not solve the issue"
                                        );
                                        break 'reload_page;
                                    } else {
                                        leptos::logging::warn!(
                            ...
                            leptos::logging::log!("Reloading");
                            if let Err(e) = location
                                .reload_with_forceget(true)
                                .or_else(|_| location.reload())
                            {
                                leptos::logging::error!(
                                    "Failed to reload: {}",
